...
class EncoderMotorController:
    ...
    def set_speed(self, speed, motor_id=None, offset=0):
        global th
        # Publish the control message to the motor driver through IIC
        with smbus2.SMBus(self.i2c_port) as bus:
            try:
                if motor_id is None:
...
# The macanum wheel chassis speed process 
...
last_angular = 0
last_direction = 90
# slow velocith process function
...
# Translation Control Callback Function
...
# Normal Conrtol Callback Function
def Set_Velocity(msg):
    ...
    if th is None: # Control the slow velocity through subthread
